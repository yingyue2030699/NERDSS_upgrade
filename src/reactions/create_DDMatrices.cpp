...
void create_DDMatrices(gsl_matrix*& survMatrix, gsl_matrix*& normMatrix, gsl_matrix*& pirMatrix,
                       double bindRadius, double Dtot, double comRMax, double ktemp,
                       const Parameters& params) {
  //    char fname[100];
  const nerdss::probability::TwoDReactionTableSpec spec{bindRadius, Dtot, comRMax, ktemp,
                                                        params.timeStep};
  double RStepSize{nerdss::probability::two_d_table_step_size(spec)};
